...
def list_components_user_count(self):
...
def log_activity_count_by_months(self):
...
def log_users_count_by_months(self):
    grouped = self.logs.groupby([self.logs.timecreated.dt.year,
                                self.logs.timecreated.dt.month])
...
def get_counts(self, df, groupby, count,ascending=False):
...
def get_activity_counts(self, username):
    self.user_logs = self.get_user_logs(username)

    condition = ((self.user_logs.edulevel == 2) &
                 (self.user_logs.courseid == 10464) &
                 (self.user_logs.username == username))

    return self.get_counts(self.user_logs[condition], ['username','eventname'], 'id')


def get_user_activity_by_date(self, df):
